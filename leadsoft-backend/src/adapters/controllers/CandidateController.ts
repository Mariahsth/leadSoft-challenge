...
import { getRavenDbConnection } from "../../infrastructure/config/ravenDbConfig";
  ...
  async register(req: Request, res: Response) {
    ...
    try {
      ...
      if (existingEmail) {
        return res
          .status(400)
          .json({ field: "email", message: "Email já cadastrado" });
      }
  ...
  async getImages(req: Request, res: Response) {
    try {
      ...
      if (!attachments || attachments.length === 0) {
        return res
          .status(404)
          .json({ message: "Nenhuma imagem encontrada para esse candidato" });
      }
